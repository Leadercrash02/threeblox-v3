...
----------------------------------------------------------------
-- FULL AUTO FISHING ENGINE - LENGKAP
----------------------------------------------------------------
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
local Events = {
    ...
    autoFishingState = Net:WaitForChild("RF/UpdateAutoFishingState"),
}
...
-- SMART GUI KILLER
----------------------------------------------------------------
local function KillFishingGui()
    pcall(function()
        for _, gui in ipairs(pg:GetChildren()) do
            if gui:IsA("ScreenGui") then
                local name = gui.Name:lower()
                if name:find("fish") or name:find("rod") or name:find("charge") or 
                   name:find("mini") or name:find("game") or name:find("bar") then
                    gui:Destroy()
                end
            end
        end
...
-- HOOK MINIGAME (V2 + KILLER)
local oldMinigame = Events.minigame.InvokeServer
Events.minigame.InvokeServer = function(self, val1, val2)
    KillFishingGui()
    return true
end
...
----------------------------------------------------------------
-- V2 FUNCTIONS (UNCHANGED)
----------------------------------------------------------------
local function Reel_V3() Events.fishing:FireServer() end
local function Cast_V3()
    Events.equip:FireServer(1)
    task.wait(0.05)
    Events.charge:InvokeServer(workspace:GetServerTimeNow())
    task.wait(0.02)
    Events.minigame:FireServer(1.2854545116425, 1)  -- V2 FireServer
end

local function UpdateAutoFishingState(enabled)
    Events.autoFishingState:InvokeServer(enabled)
end

----------------------------------------------------------------
-- V2 ENGINES (EXACT COPY + KILLER)
----------------------------------------------------------------
...
local function BlatantCycle_V2()
    ...
    Events.equip:FireServer(1)
    task.wait(0.01)
    Events.minigame:FireServer(1.2854545116425, 1)
    task.wait(0.01)
    Events.minigame:FireServer(1.2854545116425, 1)
    task.wait(0.01)
    Events.minigame:FireServer(1.2854545116425, 1)
...
-- V2 TOGGLE (EXACT)
----------------------------------------------------------------
function ToggleBlatant()
    BlatantOn = not BlatantOn
    UpdateAutoFishingState(BlatantOn)
    KillFishingGui()
    print(BlatantOn and "🟢 BLATANT V2 + NO CHARGE ON" or "🔴 OFF")
end

print("✅ BLATANT V2 FIXED - GUI KILLER ADDED")
