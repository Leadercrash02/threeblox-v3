...
-- mapping Id jenis (sesuaikan dengan data real)
local TotemTypeId = {
    Mutasi = 2,
    Shiny  = 3,
    Lucky  = 1,
...
-- global state
G.RAYAutoTotemOn = G.RAYAutoTotemOn or false
G.RAYSelectedTotems = G.RAYSelectedTotems or {} -- [UUID] = true
...
local function SpawnTotemUUID(uuid)
    ...
    -- sesuaikan sama SimpleSpy kamu
    pcall(function()
        SpawnTotemRemote:FireServer(uuid)
        -- kalau nggak spawn, ganti ke:
...
local function autoDropdown(text)
    ...
    elseif text == "Auto Totem" then
        ...
        local function refreshToggle()
            pill.BackgroundColor3 = G.RAYAutoTotemOn and ACCENT or MUTED
            knob.Position = G.RAYAutoTotemOn and UDim2.new(1,-21,0.5,-9) or UDim2.new(0,3,0.5,-9)
        end

        pill.MouseButton1Click:Connect(function()
            G.RAYAutoTotemOn = not G.RAYAutoTotemOn
            refreshToggle()
        end)
        ...
        local function rebuildTotemPanel()
            ...
            for _, info in ipairs(totems) do
                ...
                b.Size = UDim2.new(1,0,0,26)
                b.BackgroundColor3 = CARD
                b.BackgroundTransparency = G.RAYSelectedTotems[info.UUID] and 0.08 or 0.18
                b.Font = Enum.Font.Gotham
                b.TextSize = 13
                b.TextXAlignment = Enum.TextXAlignment.Left
                b.TextColor3 = TEXT
                ...
                b.MouseButton1Click:Connect(function()
                    if G.RAYSelectedTotems[info.UUID] then
                        G.RAYSelectedTotems[info.UUID] = nil
                    else
                        G.RAYSelectedTotems[info.UUID] = true
                    end
                    rebuildTotemPanel()
                end)
            end
