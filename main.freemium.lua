...
-- Status label untuk debug/info - FIX: pake Color3 langsung, bukan THEME.SUBTEXT
local ChestStatusLabel = CreateLabel(ClaimChestSection, {
    ...
    TextSize = 11,
    TextXAlignment = Enum.TextXAlignment.Left,
    TextColor3 = Color3.fromRGB(150, 150, 150), -- FIX: ganti THEME.SUBTEXT jadi Color3 langsung
    Text = "Initializing..."
})
...
--==================================================
-- CLAIM PIRATE CHEST LOGIC - FIX: pake pcall untuk Replion
--==================================================
...
-- FIX: Setup Replion dengan pcall biar gak error
task.spawn(function()
    -- Tunggu game siap dulu
    task.wait(2)
    
    -- Coba ambil Replion dengan aman
    ...
    if not success or not result or typeof(result.Data) ~= "table" then
        ...
        ChestStatusLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
        warn("[RAY] Failed to get PirateTreasureChests: " .. tostring(result))
        return
    end
...
-- Setup RemoteEvent dengan aman
local ClaimPirateChest = nil
task.spawn(function()
    local success, result = pcall(function()
        local Net = ReplicatedStorage:WaitForChild("RE", 10)
        return Net:WaitForChild("ClaimPirateChest", 10)
    end)
    
    if success and result then
        ClaimPirateChest = result
    else
        warn("[RAY] ClaimPirateChest not found!")
    end
end)
