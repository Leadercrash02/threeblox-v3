...
--==================================================
-- GLOBAL STATES (Consolidated)
--==================================================
_G.RAY = _G.RAY or {
    GhostSharkHuntActive = false,
    MegalodonHuntActive = false,
    SavedPositions = {},
    SelectedMerchantItem = nil,
    MerchantBuyQty = 1,
    MerchantAutoBuy = false,
    ChestFarmOn = false,
    ChestFarmReplionReady = false,
    LochNess = {
        CurrentState = "IDLE",
        NextEventTime = 0,
        EventEndTime = 0,
...
--==================================================
-- OPTIMIZED HELPER FUNCTIONS (Minimal Locals)
--==================================================
function CreateCorner(p, r) local c = Instance.new("UICorner", p) c.CornerRadius = UDim.new(0, r or 8) return c end
function CreateStroke(p, c, t) local s = Instance.new("UIStroke", p) s.Color, s.Thickness, s.Transparency = c or THEME.MAIN, t or 1, 0.5 return s end
function CreateLabel(p, pr) local l = Instance.new("TextLabel", p) for k, v in pairs(pr) do l[k] = v end return l end

function CreateTeleportPanel(n, i)
    ...
    CreateCorner(p, 10)
    CreateStroke(p, THEME.MAIN, 0.5)
    
    CreateLabel(p, {Size = UDim2.new(1, -10, 0, 24), Position = UDim2.new(0, 5, 0, 6), BackgroundTransparency = 1, Font = Enum.Font.GothamBold, TextSize = 16, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = THEME.TEXT, ZIndex = 11, Text = n})
    CreateLabel(p, {Size = UDim2.new(1, -10, 0, 18), Position = UDim2.new(0, 5, 0, 30), BackgroundTransparency = 1, Font = Enum.Font.Gotham, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = Color3.fromRGB(200, 200, 200), ZIndex = 11, Text = i})
    
    local s = Instance.new("ScrollingFrame", p)
    s.Size, s.Position, s.BackgroundTransparency, s.BorderSizePixel, s.ScrollBarThickness, s.AutomaticCanvasSize, s.ScrollBarImageColor3, s.ZIndex = UDim2.new(1, -10, 1, -70), UDim2.new(0, 5, 0, 54), 1, 0, 3, Enum.AutomaticSize.Y, THEME.MAIN, 10
...
function CreateListEntry(p, t, o)
    local r = Instance.new("Frame", p)
    r.Size, r.BackgroundTransparency, r.ZIndex = UDim2.new(1, -4, 0, 24), 1, 11
    
    local l = Instance.new("Frame", r)
    l.Name, l.Size, l.BackgroundColor3, l.Visible, l.ZIndex = "Highlight", UDim2.new(0, 3, 1, 0), THEME.MAIN, false, 12
    
    local b = Instance.new("TextButton", r)
    b.Size, b.Position, b.BackgroundColor3, b.TextColor3, b.Font, b.TextSize, b.TextXAlignment, b.Text, b.ZIndex = UDim2.new(1, -6, 1, 0), UDim2.new(0, 4, 0, 0), Color3.fromRGB(30, 30, 50), THEME.TEXT, Enum.Font.Gotham, 12, Enum.TextXAlignment.Left, "  " .. t, 11
...
function CreateSectionRow(p, t, b, o)
    local r = Instance.new("Frame", p)
    r.Size, r.BackgroundTransparency = UDim2.new(1, 0, 0, 36), 1
    
    CreateLabel(r, {Size = UDim2.new(1, -110, 1, 0), Position = UDim2.new(0, 16, 0, 0), BackgroundTransparency = 1, Font = Enum.Font.Gotham, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = THEME.TEXT, Text = t})
    
    local btn = Instance.new("TextButton", r)
    btn.Size, btn.Position, btn.BackgroundColor3, btn.BackgroundTransparency, btn.Text, btn.TextColor3, btn.Font, btn.TextSize = UDim2.new(0, 110, 0, 24), UDim2.new(1, -126, 0.5, -12), THEME.CARD, 0.1, b, THEME.TEXT, Enum.Font.GothamBold, 12
    
    CreateCorner(btn, 8)
    btn.MouseButton1Click:Connect(o)
    
    return r
end

function CreateTogglePill(p, d, o)
    local f = Instance.new("Frame", p)
    f.Size, f.Position, f.BackgroundColor3, f.BorderSizePixel = UDim2.new(0, 50, 0, 26), UDim2.new(1, -66, 0.5, -13), Color3.fromRGB(40, 40, 50), 0
    CreateCorner(f, 13)
    
    local c = Instance.new("Frame", f)
    c.Size, c.Position, c.BackgroundColor3, c.BorderSizePixel = UDim2.new(0, 20, 0, 20), UDim2.new(0, 3, 0.5, -10), Color3.fromRGB(255, 255, 255), 0
    CreateCorner(c, 10)
    ...
    local function u()
        if s then
            f.BackgroundColor3 = Color3.fromRGB(0, 170, 100)
            c:TweenPosition(UDim2.new(0, 27, 0.5, -10), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true)
        else
            f.BackgroundColor3 = Color3.fromRGB(70, 70, 80)
            c:TweenPosition(UDim2.new(0, 3, 0.5, -10), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true)
        end
    end
    
    u()
    
    Instance.new("TextButton", f).Size, Instance.new("TextButton", f).BackgroundTransparency, Instance.new("TextButton", f).Text = UDim2.new(1, 0, 1, 0), 1, ""
    Instance.new("TextButton", f).MouseButton1Click:Connect(function() s = not s u() if o then o(s) end end)
    
    return f, function(n) s = n u() end
end

function TableCount(t) local c = 0 for _ in pairs(t) do c = c + 1 end return c end
...
function TpToIsland(cf, l)
    local h = (Player.Character or Player.CharacterAdded:Wait()):FindFirstChild("HumanoidRootPart")
    if h then h.CFrame = cf if NotifyFeature then NotifyFeature("Teleport: " .. l, true) end end
end

islandNames = {}
for n in pairs(IslandTeleportCF) do table.insert(islandNames, n) end
table.sort(islandNames)

for _, n in ipairs(islandNames) do CreateListEntry(IslandScroll, n, function() TpToIsland(IslandTeleportCF[n], n) end) end

CreateSectionRow(IslandSection, "Teleport Island Panel", "Open", function() IslandPanel.Visible = not IslandPanel.Visible end)
...
function TpToPlayer(tp)
    if not tp or tp == Player then return end
    local m, t = (Player.Character or Player.CharacterAdded:Wait()):FindFirstChild("HumanoidRootPart"), tp.Character and tp.Character:FindFirstChild("HumanoidRootPart")
    if m and t then m.CFrame = t.CFrame if NotifyFeature then NotifyFeature("Teleport to: " .. tp.Name, true) end end
end
...
for _, plr in ipairs(Players:GetPlayers()) do if plr ~= Player then CreatePlayerEntry(plr) end end
Players.PlayerAdded:Connect(function(plr) if plr ~= Player then CreatePlayerEntry(plr) end end)
Players.PlayerRemoving:Connect(function(plr) local r = PlayerScroll:FindFirstChild("PlayerRow_" .. plr.Name) if r then r:Destroy() end end)

CreateSectionRow(PlayerSection, "Teleport Player Panel", "Open", function() PlayerPanel.Visible = not PlayerPanel.Visible end)
...
function getSharkHuntModel()
    local pr = Workspace:FindFirstChild("Props")
    if not pr then return nil end
    local m = pr:FindFirstChild("Shark Hunt") or pr:FindFirstChild("Ghost Shark Hunt")
    return (m and m:IsA("Model")) and m or nil
end

function ensureFloorUnderShark()
    local m = getSharkHuntModel()
    if not m then return nil, nil end
    if not m.PrimaryPart then
        local a = m:FindFirstChildWhichIsA("BasePart", true)
        if a then m.PrimaryPart = a end
    end
    local an = m.PrimaryPart
    if not an then return nil, nil end
    
    if not ghostSharkFloor or not ghostSharkFloor.Parent then
        ghostSharkFloor = Instance.new("Part")
        ghostSharkFloor.Name, ghostSharkFloor.Anchored, ghostSharkFloor.CanCollide, ghostSharkFloor.Transparency, ghostSharkFloor.Size, ghostSharkFloor.Material, ghostSharkFloor.Parent = "SharkHuntFloor_Client", true, true, 1, Vector3.new(80, 1, 80), Enum.Material.SmoothPlastic, Workspace
    end
    
    ghostSharkFloor.CFrame = CFrame.new(Vector3.new(an.Position.X, an.Position.Y - 2, an.Position.Z))
    return an, ghostSharkFloor
end

function TeleportToGhostShark()
    local a, f = ensureFloorUnderShark()
    if not a or not f then return end
    local c, r = Player.Character or Player.CharacterAdded:Wait(), (Player.Character or Player.CharacterAdded:Wait()):WaitForChild("HumanoidRootPart")
    r.AssemblyLinearVelocity, r.AssemblyAngularVelocity = Vector3.new(0, 0, 0), Vector3.new(0, 0, 0)
    c:PivotTo(CFrame.new(f.Position + Vector3.new(0, f.Size.Y/2 + 4, 0), f.Position + Vector3.new(0, f.Size.Y/2 + 4, 0) + a.CFrame.LookVector))
end

function TeleportToMegalodon()
    local a
    for _, o in ipairs(workspace:GetDescendants()) do if o:IsA("BasePart") and o.Name == "Megalodon Hunt" then a = o break end end
    if not a then return end
    local c, r = Player.Character or Player.CharacterAdded:Wait(), (Player.Character or Player.CharacterAdded:Wait()):WaitForChild("HumanoidRootPart")
    r.AssemblyLinearVelocity, r.AssemblyAngularVelocity = Vector3.new(0, 0, 0), Vector3.new(0, 0, 0)
    c:PivotTo(CFrame.new(a.Position + Vector3.new(0, 5, 0), a.Position + Vector3.new(0, 5, 0) + a.CFrame.LookVector))
end

-- Ghost Shark Replion
task.spawn(function()
    local ok, rp = pcall(function() return require(ReplicatedStorage.Packages.Replion) end)
    if not ok or not rp or not rp.Client then return end
    local sc, er = pcall(function() return rp.Client:WaitReplion("Events") end)
    if not sc or not er then return end
    
    local function onIns(i, n) if n == "Shark Hunt" then _G.RAY.GhostSharkHuntActive = true if NotifyFeature then NotifyFeature("Ghost Shark Hunt Spawned!", true) end end end
    local function onRem(i, n) if n == "Shark Hunt" then _G.RAY.GhostSharkHuntActive = false if ghostSharkFloor then ghostSharkFloor:Destroy() ghostSharkFloor = nil end if NotifyFeature then NotifyFeature("Ghost Shark Hunt Ended", false) end end end
    
    er:OnArrayInsert("Events", onIns)
    er:OnArrayRemove("Events", onRem)
    
    for i, n in ipairs(er:Get("Events") or {}) do onIns(i, n) end
end)
...
CreateListEntry(EventHuntScroll, "Ghost Shark Hunt", function()
    if not _G.RAY.GhostSharkHuntActive then if NotifyFeature then NotifyFeature("Ghost Shark Hunt not active!", false) end return end
...
CreateListEntry(EventHuntScroll, "Megalodon Hunt", function()
    if not _G.RAY.MegalodonHuntActive then if NotifyFeature then NotifyFeature("Megalodon Hunt not found!", false) end return end
    TeleportToMegalodon()
...
task.spawn(function()
    while true do
        for _, e in ipairs(EventHuntScroll:GetChildren()) do
            if e:IsA("Frame") then
                local b, h = e:FindFirstChildOfClass("TextButton"), e:FindFirstChild("Highlight")
                if b and h then
                    if b.Text:find("Ghost Shark") then
                        if _G.RAY.GhostSharkHuntActive then b.BackgroundColor3, h.Visible = Color3.fromRGB(40, 70, 40), true else b.BackgroundColor3, h.Visible = Color3.fromRGB(30, 30, 50), false end
                    elseif b.Text:find("Megalodon") then
                        local ex = false
                        for _, o in ipairs(workspace:GetDescendants()) do if o:IsA("BasePart") and o.Name == "Megalodon Hunt" then ex = true break end end
                        _G.RAY.MegalodonHuntActive = ex
                        if ex then b.BackgroundColor3, h.Visible = Color3.fromRGB(40, 70, 40), true else b.BackgroundColor3, h.Visible = Color3.fromRGB(30, 30, 50), false end
                    end
                end
            end
        end
...
CreateSectionRow(EventHuntSection, "Event Hunt Panel", "Open", function() EventHuntPanel.Visible = not EventHuntPanel.Visible end)

--==================================================
-- LOCH NESS EVENT
--==================================================
ANCIENT_RUIN_CF = CFrame.new(6082.87842, -585.924316, 4633.71631, -0.681475937, 0, 0.731840551, 0, 1, 0, -0.731840551, 0, -0.681475937)
EVENT_DURATION_MINUTES, EVENT_HOURS_UTC = 10, {0, 4, 8, 12, 16, 20}
...
function GetNextEventStartUTC()
    ...
    for _, h in ipairs(EVENT_HOURS_UTC) do if h * 60 > nm then return os.time({year = n.year, month = n.month, day = n.day, hour = h, min = 0, sec = 0, isdst = false}) end end
    return os.time({year = n.year, month = n.month, day = n.day + 1, hour = EVENT_HOURS_UTC[1], min = 0, sec = 0, isdst = false})
end

function FormatTime(s) s = math.max(0, math.floor(s)) return string.format("%02d:%02d:%02d", math.floor(s / 3600), math.floor((s % 3600) / 60), s % 60) end
...
function ReturnToSavedPosition()
    ...
    if not d then if NotifyFeature then NotifyFeature("No saved position!", false) end return end
    local c, h = Player.Character or Player.CharacterAdded:Wait(), (Player.Character or Player.CharacterAdded:Wait()):FindFirstChild("HumanoidRootPart")
    if not h then return end
...
function UpdateLochNessState()
    ...
    if s.CurrentState == EVENT_STATE.IDLE then
        if n >= s.NextEventTime then s.CurrentState, s.EventEndTime, s.IsAutoTeleported, s.IsReturned = EVENT_STATE.ACTIVE, n + (EVENT_DURATION_MINUTES * 60), false, false return "EVENT_START" end
    elseif s.CurrentState == EVENT_STATE.ACTIVE then
        if n >= s.EventEndTime then s.CurrentState = EVENT_STATE.ENDED return "EVENT_END" end
    elseif s.CurrentState == EVENT_STATE.ENDED then s.NextEventTime, s.CurrentState, s.IsAutoTeleported, s.IsReturned = GetNextEventStartUTC(), EVENT_STATE.IDLE, false, false return "RESET" end
    return nil
end

function GetLochNessDisplayTime()
    local n, s = os.time(), _G.RAY.LochNess
    if s.CurrentState == EVENT_STATE.IDLE then return "Next Event:", s.NextEventTime - n, Color3.fromRGB(0, 255, 140)
    elseif s.CurrentState == EVENT_STATE.ACTIVE then return "Event Ends:", s.EventEndTime - n, Color3.fromRGB(255, 140, 0)
    else return "Event Ended!", 0, Color3.fromRGB(255, 80, 80) end
end

function HandleLochNessAutoActions()
    local s = _G.RAY.LochNess
    if s.CurrentState == EVENT_STATE.ACTIVE and s.AutoTeleportEnabled and not s.IsAutoTeleported then if SaveCurrentPosition() then task.wait(0.5) TeleportToAncientRuin() s.IsAutoTeleported = true end end
    if s.CurrentState == EVENT_STATE.ENDED and s.AutoTeleportEnabled and s.IsAutoTeleported and not s.IsReturned then task.wait(1) ReturnToSavedPosition() s.IsReturned = true end
end
...
TimerRow = Instance.new("Frame", LochNessSection) TimerRow.Size, TimerRow.BackgroundTransparency = UDim2.new(1, 0, 0, 40), 1
...
TeleportRow = Instance.new("Frame", LochNessSection) TeleportRow.Size, TeleportRow.BackgroundTransparency = UDim2.new(1, 0, 0, 36), 1
...
TeleportBtn.MouseButton1Click:Connect(function() if _G.RAY.LochNess.CurrentState == EVENT_STATE.ACTIVE and not _G.RAY.LochNess.SavedPosition then SaveCurrentPosition() end TeleportToAncientRuin() end)

ReturnRow = Instance.new("Frame", LochNessSection) ReturnRow.Size, ReturnRow.BackgroundTransparency = UDim2.new(1, 0, 0, 36), 1
...
AutoTeleportRow = Instance.new("Frame", LochNessSection) AutoTeleportRow.Size, AutoTeleportRow.BackgroundTransparency = UDim2.new(1, 0, 0, 36), 1
...
AutoTeleportToggle, SetAutoTeleportState = CreateTogglePill(AutoTeleportRow, false, function(s) _G.RAY.LochNess.AutoTeleportEnabled = s if NotifyFeature then NotifyFeature("Loch Ness Auto: " .. (s and "ON" or "OFF"), s) end end)

-- Loch Ness Loop
_G.RAY.LochNess.NextEventTime = GetNextEventStartUTC()

task.spawn(function()
    while true do
        local sc = UpdateLochNessState()
        HandleLochNessAutoActions()
        
        local lt, tv, tc = GetLochNessDisplayTime()
        TimerLabel.Text, TimeDisplay.Text, TimeDisplay.TextColor3 = lt, FormatTime(tv), tc
        
        local st = _G.RAY.LochNess.CurrentState
        StatusLabel.Text, StatusLabel.TextColor3 = st, (st == EVENT_STATE.IDLE and Color3.fromRGB(100, 200, 100) or (st == EVENT_STATE.ACTIVE and Color3.fromRGB(255, 200, 100) or Color3.fromRGB(255, 100, 100)))
        
        if sc and NotifyFeature then if sc == "EVENT_START" then NotifyFeature("Loch Ness Event STARTED!", true) elseif sc == "EVENT_END" then NotifyFeature("Loch Ness Event ENDED!", false) end end
...
ChestFarmStatusRow = Instance.new("Frame", ChestFarmSection) ChestFarmStatusRow.Size, ChestFarmStatusRow.BackgroundTransparency = UDim2.new(1, 0, 0, 36), 1
...
ChestFarmToggle, SetChestFarmState = CreateTogglePill(ChestFarmStatusRow, _G.RAY.ChestFarmOn, function(s) _G.RAY.ChestFarmOn = s if NotifyFeature then NotifyFeature("Chest Farm: " .. (s and "ON" or "OFF"), s) end end)
...
ChestFarmStatsRow = Instance.new("Frame", ChestFarmSection) ChestFarmStatsRow.Size, ChestFarmStatsRow.BackgroundTransparency = UDim2.new(1, 0, 0, 24), 1
...
ClaimPirateChest, chestReplion = NetFolder:WaitForChild("RE/ClaimPirateChest"), nil

task.spawn(function()
    local ok, r = pcall(function() return Replion.Client:WaitReplion("PirateTreasureChests") end)
    if not ok or not r or typeof(r.Data) ~= "table" then ChestFarmInfoLabel.Text, ChestFarmInfoLabel.TextColor3 = "Replion failed", Color3.fromRGB(255, 100, 100) return end
    chestReplion, _G.RAY.ChestFarmReplionReady, ChestFarmInfoLabel.Text, ChestFarmInfoLabel.TextColor3 = r, true, "Replion connected", Color3.fromRGB(100, 255, 100)
end)

function GetAllChestUUIDs()
    if not chestReplion then return {} end
    local s = chestReplion.Data and chestReplion.Data.SpawnedChests
    if typeof(s) ~= "table" then return {} end
    local l = {}
    for _, e in ipairs(s) do if typeof(e.Id) == "string" then table.insert(l, e.Id) end end
    return l
end
...
task.spawn(function()
    while true do
        task.wait(0.25)
        if not _G.RAY.ChestFarmReplionReady then ChestFarmInfoLabel.Text = "Connecting to Replion..." continue end
        if not _G.RAY.ChestFarmOn then ChestFarmInfoLabel.Text, ChestFarmInfoLabel.TextColor3 = "Paused", Color3.fromRGB(150, 150, 150) continue end
        local u = GetAllChestUUIDs()
        ChestCountLabel.Text = "Chests: " .. #u
        if #u == 0 then ChestFarmInfoLabel.Text, ChestFarmInfoLabel.TextColor3 = "No chests spawned", Color3.fromRGB(255, 200, 100)
        else ChestFarmInfoLabel.Text, ChestFarmInfoLabel.TextColor3 = "Claiming " .. #u .. " chests...", Color3.fromRGB(0, 255, 140) for _, i in ipairs(u) do pcall(function() ClaimPirateChest:FireServer(i) end) end end
    end
end)
...
function SavePosition(n)
    ...
    if not cf then if NotifyFeature then NotifyFeature("Failed to save position!", false) end return false end
    _G.RAY.SavedPositions[n] = {Position = cf.Position, LookVector = cf.LookVector, UpVector = cf.UpVector, Time = os.time()}
    if NotifyFeature then NotifyFeature("Saved position: " .. n, true) end
    return true
end

function TeleportToSaved(n)
    local d = _G.RAY.SavedPositions[n]
    if not d then if NotifyFeature then NotifyFeature("Position not found: " .. n, false) end return end
    local h = (Player.Character or Player.CharacterAdded:Wait()):FindFirstChild("HumanoidRootPart")
    if not h then return end
    h.CFrame = CFrame.new(d.Position, d.Position + d.LookVector)
...
function DeleteSavedPosition(n)
...
task.spawn(function()
    task.wait(2)
    local ls, lt = nil, 0
    for n, d in pairs(_G.RAY.SavedPositions) do if d.Time and d.Time > lt then lt, ls = d.Time, n end end
    if ls then TeleportToSaved(ls) end
end)
...
function RefreshSavedPositionsList()
    for _, c in ipairs(SavedPosScroll:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
    local ns = {}
    for n in pairs(_G.RAY.SavedPositions) do table.insert(ns, n) end
    table.sort(ns)
    for _, n in ipairs(ns) do
        ...
        local b = Instance.new("TextButton", r)
        b.Size, b.Position, b.BackgroundColor3, b.TextColor3, b.Font, b.TextSize, b.TextXAlignment, b.Text, b.ZIndex = UDim2.new(1, -30, 1, 0), UDim2.new(0, 4, 0, 0), Color3.fromRGB(30, 30, 50), THEME.TEXT, Enum.Font.Gotham, 12, Enum.TextXAlignment.Left, "  " .. n, 11
        CreateCorner(b, 6)
        
        local d = Instance.new("TextButton", r)
        d.Size, d.Position, d.BackgroundColor3, d.TextColor3, d.Font, d.TextSize, d.Text, d.ZIndex = UDim2.new(0, 20, 0, 20), UDim2.new(1, -26, 0.5, -10), Color3.fromRGB(80, 30, 30), Color3.fromRGB(255, 150, 150), Enum.Font.GothamBold, 12, "X", 11
        CreateCorner(d, 4)
        
        b.MouseButton1Click:Connect(function() TeleportToSaved(n) for _, c in ipairs(SavedPosScroll:GetChildren()) do local h = c:FindFirstChild("Highlight") if h then h.Visible = (c == r) end end end)
        d.MouseButton1Click:Connect(function() DeleteSavedPosition(n) RefreshSavedPositionsList() end)
    end
end
...
do
    ...
    CreateLabel(r, {Size = UDim2.new(0.5, -20, 1, 0), Position = UDim2.new(0, 16, 0, 0), BackgroundTransparency = 1, Font = Enum.Font.Gotham, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = THEME.TEXT, Text = "Save Current Pos"})
    
    local nb = Instance.new("TextBox", r)
    nb.Size, nb.Position, nb.BackgroundColor3, nb.BackgroundTransparency, nb.Text, nb.TextColor3, nb.Font, nb.TextSize, nb.ClearTextOnFocus = UDim2.new(0.3, 0, 0, 24), UDim2.new(0.5, -10, 0.5, -12), THEME.CARD, 0.12, "Pos " .. (TableCount(_G.RAY.SavedPositions) + 1), THEME.TEXT, Enum.Font.Gotham, 12, true
    CreateCorner(nb, 6)
    
    local sb = Instance.new("TextButton", r)
    sb.Size, sb.Position, sb.BackgroundColor3, sb.TextColor3, sb.Font, sb.TextSize, sb.Text = UDim2.new(0, 60, 0, 24), UDim2.new(1, -70, 0.5, -12), Color3.fromRGB(40, 70, 40), THEME.TEXT, Enum.Font.GothamBold, 12, "Save"
    CreateCorner(sb, 6)
    sb.MouseButton1Click:Connect(function()
        local nm = nb.Text:gsub("^%s+", ""):gsub("%s+$", "")
        if #nm == 0 then nm = "Pos " .. (TableCount(_G.RAY.SavedPositions) + 1) end
        if SavePosition(nm) then RefreshSavedPositionsList() nb.Text = "Pos " .. (TableCount(_G.RAY.SavedPositions) + 1) end
    end)
end

CreateSectionRow(SavedPosSection, "Saved Positions Panel", "Open", function() SavedPosPanel.Visible = not SavedPosPanel.Visible if SavedPosPanel.Visible then RefreshSavedPositionsList() end end)
...
function IsInsidePanel(p, pos)
    if not p or not p.Visible then return false end
    return (pos.X >= p.AbsolutePosition.X and pos.X <= p.AbsolutePosition.X + p.AbsoluteSize.X and pos.Y >= p.AbsolutePosition.Y and pos.Y <= p.AbsolutePosition.Y + p.AbsoluteSize.Y)
end

function IsInsideSection(s, pos)
    if not s then return false end
    return (pos.X >= s.AbsolutePosition.X and pos.X <= s.AbsolutePosition.X + s.AbsoluteSize.X and pos.Y >= s.AbsolutePosition.Y and pos.Y <= s.AbsolutePosition.Y + s.AbsoluteSize.Y)
end
...
UIS.InputBegan:Connect(function(i)
    ...
    for _, p in ipairs(AllPanels) do if IsInsidePanel(p, pos) then return end end
    for _, s in ipairs({IslandSection, PlayerSection, EventHuntSection, LochNessSection, ChestFarmSection, SavedPosSection}) do if IsInsideSection(s, pos) then return end end
    for _, p in ipairs(AllPanels) do if p and p.Visible then p.Visible = false end end
end)
...
MERCHANT_ITEM_MAP = {}
for _, i in ipairs(MarketItemData) do MERCHANT_ITEM_MAP[i.Id] = i end

function GetCurrentMerchantStock()
    local ids, stock = MerchantReplion:GetExpect("Items") or {}, {}
    for _, id in ipairs(ids) do
        local d = MERCHANT_ITEM_MAP[id]
        if d then table.insert(stock, {Id = d.Id, Name = d.Identifier or d.Name or ("Item_" .. id), Price = d.Price or 0, Currency = d.Currency or "Coins", MaxStock = d.MaxStock or 1, Data = d}) end
    end
    return stock
end

function BuyMerchantItem(id, q)
    q = math.max(1, tonumber(q) or 1)
    for i = 1, q do task.spawn(function() pcall(function() PurchaseMarketItemRF:InvokeServer(id) end) end) task.wait(0.1) end
    return true
end
...
StatusRow = Instance.new("Frame", MerchantSection) StatusRow.Size, StatusRow.BackgroundTransparency = UDim2.new(1, 0, 0, 30), 1
...
SelectedRow = Instance.new("Frame", MerchantSection) SelectedRow.Size, SelectedRow.BackgroundTransparency = UDim2.new(1, 0, 0, 30), 1
...
QuantityRow = Instance.new("Frame", MerchantSection) QuantityRow.Size, QuantityRow.BackgroundTransparency = UDim2.new(1, 0, 0, 36), 1
...
qtyBox.Size, qtyBox.Position, qtyBox.BackgroundColor3, qtyBox.BackgroundTransparency, qtyBox.Text, qtyBox.TextColor3, qtyBox.Font, qtyBox.TextSize, qtyBox.ClearTextOnFocus = UDim2.new(0, 60, 0, 24), UDim2.new(0.5, -10, 0.5, -12), THEME.CARD, 0.1, tostring(_G.RAY.MerchantBuyQty), THEME.TEXT, Enum.Font.Gotham, 12, false
CreateCorner(qtyBox, 8)
qtyBox.FocusLost:Connect(function() local n = tonumber(qtyBox.Text) if not n or n < 1 then n = 1 qtyBox.Text = "1" end _G.RAY.MerchantBuyQty = math.min(n, 99) end)

buyBtn = Instance.new("TextButton", QuantityRow)
buyBtn.Size, buyBtn.Position, buyBtn.BackgroundColor3, buyBtn.TextColor3, buyBtn.Font, buyBtn.TextSize, buyBtn.Text = UDim2.new(0, 80, 0, 24), UDim2.new(1, -90, 0.5, -12), Color3.fromRGB(40, 100, 40), Color3.fromRGB(255, 255, 255), Enum.Font.GothamBold, 12, "BUY"
CreateCorner(buyBtn, 8)
buyBtn.MouseButton1Click:Connect(function()
    if not _G.RAY.SelectedMerchantItem then if NotifyFeature then NotifyFeature("No item selected!", false) end return end
    BuyMerchantItem(_G.RAY.SelectedMerchantItem.Id, _G.RAY.MerchantBuyQty or 1)
    if NotifyFeature then NotifyFeature("Buying " .. _G.RAY.SelectedMerchantItem.Name .. " x" .. (_G.RAY.MerchantBuyQty or 1), true) end
end)

AutoBuyRow = Instance.new("Frame", MerchantSection) AutoBuyRow.Size, AutoBuyRow.BackgroundTransparency = UDim2.new(1, 0, 0, 36), 1
...
AutoBuyToggle, SetAutoBuyState = CreateTogglePill(AutoBuyRow, _G.RAY.MerchantAutoBuy, function(s) _G.RAY.MerchantAutoBuy = s if NotifyFeature then NotifyFeature("Merchant Auto Buy: " .. (s and "ON" or "OFF"), s) end end)

CreateSectionRow(MerchantSection, "Merchant Stock Panel", "Open", function() MerchantPanel.Visible = not MerchantPanel.Visible if MerchantPanel.Visible then rebuildMerchantPanel() end end)

function rebuildMerchantPanel()
    for _, c in ipairs(MerchantScroll:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
    
    local stock = GetCurrentMerchantStock()
    
    if #stock == 0 then
        MerchantStatus.Text, MerchantStatus.TextColor3 = "Not Available", Color3.fromRGB(255, 100, 100)
        local er = Instance.new("Frame", MerchantScroll)
        er.Size, er.BackgroundTransparency = UDim2.new(1, -4, 0, 60), 1
        CreateLabel(er, {Size = UDim2.new(1, -10, 1, 0), Position = UDim2.new(0, 5, 0, 0), BackgroundTransparency = 1, Font = Enum.Font.Gotham, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Center, TextColor3 = Color3.fromRGB(150, 150, 150), Text = "No merchant stock available.\nCheck back later!"})
        _G.RAY.SelectedMerchantItem, SelectedItemLabel.Text, SelectedItemLabel.TextColor3 = nil, "None", Color3.fromRGB(150, 150, 150)
        return
    end
    
    MerchantStatus.Text, MerchantStatus.TextColor3 = #stock .. " Items", Color3.fromRGB(0, 255, 140)
    
    for _, it in ipairs(stock) do
        local r = Instance.new("Frame", MerchantScroll)
        r.Size, r.BackgroundTransparency, r.ZIndex = UDim2.new(1, -4, 0, 40), 1, 11
        
        local l = Instance.new("Frame", r)
        l.Name, l.Size, l.Position, l.BackgroundColor3, l.BorderSizePixel, l.Visible, l.ZIndex = "Highlight", UDim2.new(0, 3, 1, 0), UDim2.new(0, 0, 0, 0), THEME.MAIN, 0, (_G.RAY.SelectedMerchantItem and _G.RAY.SelectedMerchantItem.Id == it.Id), 12
        
        local b = Instance.new("TextButton", r)
        b.Size, b.Position, b.BackgroundColor3, b.TextColor3, b.Font, b.TextSize, b.TextXAlignment, b.TextYAlignment, b.Text, b.ZIndex = UDim2.new(1, -6, 1, 0), UDim2.new(0, 6, 0, 0), Color3.fromRGB(30, 30, 50), THEME.TEXT, Enum.Font.Gotham, 11, Enum.TextXAlignment.Left, Enum.TextYAlignment.Top, string.format("  %s\n  💰 %d %s", it.Name, it.Price, it.Currency), 11
        CreateCorner(b, 6)
        
        if it.Currency:lower():find("robux") or it.Currency:lower():find("premium") then b.TextColor3 = Color3.fromRGB(255, 200, 100) end
        
        b.MouseButton1Click:Connect(function()
            _G.RAY.SelectedMerchantItem = it
            SelectedItemLabel.Text, SelectedItemLabel.TextColor3 = it.Name, Color3.fromRGB(0, 255, 140)
            for _, c in ipairs(MerchantScroll:GetChildren()) do if c:IsA("Frame") then local h = c:FindFirstChild("Highlight") if h then h.Visible = (c == r) end end end
            if NotifyFeature then NotifyFeature("Selected: " .. it.Name .. " (" .. it.Price .. " " .. it.Currency .. ")", true) end
        end)
    end
end
...
MerchantReplion:OnChange("Items", function()
    ...
    if #stock > 0 then
        MerchantStatus.Text, MerchantStatus.TextColor3 = #stock .. " Items", Color3.fromRGB(0, 255, 140)
        if _G.RAY.MerchantAutoBuy and _G.RAY.SelectedMerchantItem then
            local sa = false
            for _, it in ipairs(stock) do if it.Id == _G.RAY.SelectedMerchantItem.Id then sa = true break end end
            if sa then BuyMerchantItem(_G.RAY.SelectedMerchantItem.Id, _G.RAY.MerchantBuyQty or 1) if NotifyFeature then NotifyFeature("Auto-bought " .. _G.RAY.SelectedMerchantItem.Name, true) end end
        end
    else
        MerchantStatus.Text, MerchantStatus.TextColor3, _G.RAY.SelectedMerchantItem, SelectedItemLabel.Text, SelectedItemLabel.TextColor3 = "Not Available", Color3.fromRGB(255, 100, 100), nil, "None", Color3.fromRGB(150, 150, 150)
    end
end)
...
task.spawn(function()
    while true do
        if _G.RAY.MerchantAutoBuy and _G.RAY.SelectedMerchantItem then
            for _, it in ipairs(GetCurrentMerchantStock()) do if it.Id == _G.RAY.SelectedMerchantItem.Id then BuyMerchantItem(it.Id, _G.RAY.MerchantBuyQty or 1) break end end
        end
        task.wait(3)
    end
end)
